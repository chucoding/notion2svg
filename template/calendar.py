                ...
                if notion_pages.get(date) is not None :
                    for k, notion_page in enumerate(notion_pages.get(date)) :
                        ...
                        if end_date is not None : 
                            ...
                            use_stack = False
                        notion_page_name = (bytes(notion_page["name"], 'utf-8')[0:12*int(width/120)+12*int(width/120-1)]).decode('utf-8')+"..." if len(bytes(notion_page["name"], 'utf-8')) > 12*int(width/120)+12*int(width/120-1) else notion_page["name"]
                        ...
                        if notion_page['icon'] is not None :
                            if notion_page['icon']['type'] == "emoji" :
                                ...
                                svg_days += "<image x='%d' y='%d' width='15' height='15' href='%s' />" % (120*(j)+6, (68*(i)+112+alpha),notion_page['icon']['file']['url'].replace("&","&amp;"))
